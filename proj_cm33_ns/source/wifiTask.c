...
#define MAX_TRACKED_SSIDS                            (64U)
#define WIFI_IPC_TX_QUEUE_LEN                        (32U)
...
static char scanned_ssids[MAX_TRACKED_SSIDS][CY_WCM_MAX_SSID_LEN + 1U];
static uint32_t scanned_ssid_count = RESET_VAL;
static QueueHandle_t wifi_ipc_tx_queue = NULL;
static volatile bool ipc_tx_in_flight = false;
...
CY_SECTION_SHAREDMEM static ipc_msg_t wifi_data_ipc_msg;
/*******************************************************************************
* Function Definitions
*******************************************************************************/

static void wifi_ipc_release_callback(void)
{
    ipc_tx_in_flight = false;
}

static void wifi_ipc_try_send_queued_messages(void)
{
    if ((NULL == wifi_ipc_tx_queue) || (true == ipc_tx_in_flight))
    {
        return;
    }

    while (false == ipc_tx_in_flight)
    {
        ipc_msg_t pending_msg;
        cy_en_ipc_pipe_status_t pipe_send_status;

        if (pdPASS != xQueuePeek(wifi_ipc_tx_queue, &pending_msg, 0U))
        {
            break;
        }

        /* Keep the message in shared memory while IPC owns the pointer. */
        wifi_data_ipc_msg = pending_msg;
        ipc_tx_in_flight = true;
        pipe_send_status = Cy_IPC_Pipe_SendMessage(CM55_IPC_PIPE_EP_ADDR,
                                                   CM33_IPC_PIPE_EP_ADDR,
                                                   &wifi_data_ipc_msg,
                                                   wifi_ipc_release_callback);

        if (CY_IPC_PIPE_SUCCESS == pipe_send_status)
        {
            (void)xQueueReceive(wifi_ipc_tx_queue, &pending_msg, 0U);
        }
        else
        {
            ipc_tx_in_flight = false;

            if (CY_IPC_PIPE_ERROR_SEND_BUSY == pipe_send_status)
            {
                break;
            }

            (void)xQueueReceive(wifi_ipc_tx_queue, &pending_msg, 0U);
            printf("cm33-->cm55 queued IPC send failed %lu\n", (unsigned long)pipe_send_status);
        }
    }
}
...
static void print_scan_result(cy_wcm_scan_result_t *result)
{
    ...
    uint32_t index;
    size_t ssid_len = strnlen((const char *)result->SSID, CY_WCM_MAX_SSID_LEN);

    if (RESET_VAL == ssid_len)
    {
        return;
    }

    for (index = RESET_VAL; index < scanned_ssid_count; index++)
    {
        if (0 == strcmp(scanned_ssids[index], (const char *)result->SSID))
        {
            return;
        }
    }

    if (scanned_ssid_count < MAX_TRACKED_SSIDS)
    {
        memcpy(scanned_ssids[scanned_ssid_count], result->SSID, ssid_len);
        scanned_ssids[scanned_ssid_count][ssid_len] = '\0';
        scanned_ssid_count++;
    }
    ...
    queued_msg.wifi_scan_result.security = (uint32_t)result->security;

    if (pdPASS == xQueueSend(wifi_ipc_tx_queue, &queued_msg, 0U))
    {
        wifi_ipc_try_send_queued_messages();
    ...
    {
        printf("cm33-->cm55 wifi list IPC queue full, drop SSID: %s\n", result->SSID);
    }
}
            ...
            cy_wcm_scan_status_t status)
{
    ...
    {
        /* Print and send only if SSID has not appeared in this scan cycle. */
    ...
    //     /* Reset the number of scan results to 0 for the next scan.*/
        ...
        scanned_ssid_count = RESET_VAL;
        scanned = true;
        ipc_msg_t queued_msg;
        queued_msg.client_id = CM55_IPC_PIPE_CLIENT_ID;
        queued_msg.intr_mask = CY_IPC_CYPIPE_INTR_MASK_EP1;
        queued_msg.cmd = SCAN_COMPLETE;
        xQueueSend(wifi_ipc_tx_queue, &queued_msg, 0U);
        if (pdPASS == xQueueSend(wifi_ipc_tx_queue, &queued_msg, 0U))
        {
            wifi_ipc_try_send_queued_messages();
            xQueueReset(wifi_ipc_tx_queue);
        }
        else
        {
            printf("cm33-->cm55 wifi list IPC queue full");
        }
    //     //printf("\x1b[2J\x1b[;H");
...
void scan_task(void *arg)
{
    ...
    wifi_ipc_tx_queue = xQueueCreate(WIFI_IPC_TX_QUEUE_LEN, sizeof(ipc_msg_t));
    if (NULL == wifi_ipc_tx_queue)
    {
        handle_app_error();
    }
